...
sec_directory = pathlib.Path.cwd().joinpath('data/xbrl-documents')

# Define the file path.
file_htm = sec_directory.joinpath('axp-20221231.htm').resolve()
file_cal = sec_directory.joinpath('axp-20221231_cal.xml').resolve()
file_lab = sec_directory.joinpath('axp-20221231_lab.xml').resolve()
file_def = sec_directory.joinpath('axp-20221231_def.xml').resolve()
        ...
        for cnx_item in element.iter():
            for att in cnx_item.attrib:
                if att:
                    ...
                    if not pd.isna(cnx_item.text) and cnx_item.text.strip() != '':
